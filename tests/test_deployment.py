...
def test_the_interface_asks_the_server_what_it_is_running():
    ...
    string as "no API", so the interface never calls /v1/capabilities and
    badges every deployment, real models included, as a demo. It looks like a
    cosmetic bug and silently makes the honesty badge meaningless."""
    source = (ROOT / "app" / "web" / "app.js").read_text(encoding="utf-8")

    assert "if (!API)" not in source, "empty-string base URL is falsy: compare against null"
    assert "API === null" in source
